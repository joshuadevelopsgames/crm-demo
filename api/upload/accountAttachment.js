/**
 * API endpoint for uploading account attachment files to Supabase Storage
 */
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { file, fileName, accountId, userId, userEmail } = req.body;
    
    if (!file || !fileName || !accountId || !userId) {
      return res.status(400).json({
        success: false,
        error: 'file, fileName, accountId, and userId are required'
      });
    }
    ...
    const bucketExists = buckets?.some(b => b.id === 'account-attachments');
    if (!bucketExists) {
      return res.status(400).json({
        success: false,
        error: 'Storage bucket "account-attachments" does not exist. Please create it in Supabase Dashboard → Storage.'
      });
    }
