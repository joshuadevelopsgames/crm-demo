...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      const { task_id, user_id, user_email, file_name, file_url, file_size, file_type, storage_path } = req.body;
      
      if (!task_id || !user_id || !file_name || !file_url) {
        return res.status(400).json({
          success: false,
          error: 'task_id, user_id, file_name, and file_url are required'
        });
      }
