-- Complete script to set up test user (test@test.com)
...
-- Step 2: Create or update the profile for test@test.com
...
SET role = 'admin', email = 'test@test.com', full_name = COALESCE(profiles.full_name, 'Test User');
...
SELECT 
  ...
  END as status
FROM profiles p
WHERE p.email = 'test@test.com';

-- If no rows returned, the user hasn't been created in auth.users yet
-- Go to Authentication > Users > Add user and create test@test.com first
